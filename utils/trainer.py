...
class Trainer:
    def __init__(self, model, optimizer, session):
        self.model, self.optimizer, self.session, self.print = (
            model,
            optimizer,
            session,
            defaultdict(list),
    ...
    def train(
        ...
        if _train_step is None:
            ...
            def _train_step(
                step, steps, epoch, epochs, min_epochs, model, optimizer, batch_dim
            ):
                return self.session.run(
                    train_fetch_dict(
                        step, steps, epoch, epochs, min_epochs, model, optimizer
                    ),
        ...
        if _eval_step is None:
            ...
            def _eval_step(
                ...
                eta = (
                    timedelta(
                        seconds=int(
                            (time.time() - start_time) * (epochs - epoch) / epoch
                        )
                    )
                ...
                if eval_batch is not None:
                    ...
                    for i in range(eval_batch):
                        for k, v in self.session.run(
                            eval_fetch_dict(
                                epoch, epochs, min_epochs, model, optimizer
                            ),
                            feed_dict=eval_feed_dict(
                                epoch, epochs, min_epochs, model, optimizer, batch_dim
                            ),
                        ).items():
                            output[k].append(v)
                        # pr.update(i + 1)
                    ...
                    output = self.session.run(
                        eval_fetch_dict(epoch, epochs, min_epochs, model, optimizer),
                        feed_dict=eval_feed_dict(
                            epoch, epochs, min_epochs, model, optimizer, batch_dim
                        ),
                    )
        ...
        if _test_step is None:
            ...
            def _test_step(
                model, optimizer, batch_dim, test_batch, start_time, _test_update
            ):
                self.load(directory)
                from_start = timedelta(seconds=int((time.time() - start_time)))
        ...
        for epoch in range(epochs + 1):
            if not (skip_first_eval and epoch == 0):
                ...
                if best_fn is not None and (
                    ...
                    else best_fn(result) > best_model_value
                ):
                    self.save(directory)
        ...
        _test_step(
            self.model, self.optimizer, batch_dim, eval_batch, start_time, _test_update
        )
